...
const Header = () => {
	const { title, subtitle, type, doGoBack, logoSize } =
	...
	return (
		<>
			{type !== "null" && (
				...
				>
					{doGoBack && location.pathname !== "/dubai-app/welcome" && (
						<ArrowLefContainer onClick={doGoBack}>
							<ArrowLeft />
						</ArrowLefContainer>
					)}
					<TitleContainer>
						...
						{type === "primary" && subtitle && (
							<>
								<HeaderTitle>{title}</HeaderTitle>
								<TitleBlack>{subtitle}</TitleBlack>
							</>
						)}
						{type === "primaryBlue" && subtitle && (
							<>
								<HeaderTitle>{title}</HeaderTitle>
								<TitleBlue>{subtitle}</TitleBlue>
							</>
						)}
						{type === "secondary" && (
							<>
								<LogoImg
									...
									size={logoSize}
								/>
								<TitlePurple>{subtitle}</TitlePurple>
							</>
						)}
						{type === "tertiary" && (
							<Link to="http://www.virufy.org/en/" target="_blank">
								<LogoImg
									srcSet={`${Logo}, ${Logo2x} 2x, ${Logo3x} 3x`}
									src={LogoImg}
									size={
										location.pathname !== "/dubai-app/welcome"
											? "regular"
											: "big"
									}
								/>
