	...
	React.useEffect(() => {
		...
		const checkFileConsistencyProblem = (inputState: Record<string, any>) => {
			...
			if (!inputState.welcome?.patientId && inputState[StoreKey]) {
				const { recordYourCough, recordYourSpeech, recordYourBreath } =
					inputState[StoreKey];
				const toTest = [];
				...
				if (recordYourBreath) {
					...
					if (uploadedFile) {
						toTest.push({
							file: uploadedFile,
							route: "/step-manual-upload/breath"
						});
					}
				}
				...
				if (recordYourSpeech) {
					...
					if (uploadedFile) {
						toTest.push({
							file: uploadedFile,
							route: "/step-manual-upload/speech"
						});
					}
				}

				const itemWithProblem = toTest.find((toTestItem) =>
					checkFileProblem(toTestItem.file)
				);
				if (itemWithProblem) {
		...
		if (problemRoute) {
			history.push(`/disease-app/${StoreKey}${problemRoute}`);
		}
