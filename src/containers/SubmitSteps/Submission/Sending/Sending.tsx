...
const Sending = (p: Wizard.StepProps) => {
	...
	const sendDataToBackend = useCallback(async (): Promise<void> => {
		try {
			...
			const coughFile =
				state["submit-steps"]?.recordYourCough?.recordingFile ||
				state["submit-steps"]?.recordYourCough?.uploadedFile;
	...
	const mocksendDataToBackend = useCallback(() => {
		...
		console.log(
			"patientId",
			state["submit-steps"]?.patientId,
			"Conditions",
			state["submit-steps"]?.currentMedicalCondition,
			"location",
			state["submit-steps"]?.location["lat"],
			"location",
			state["submit-steps"]?.location["lng"],
			"sick",
			state["submit-steps"]?.sick,
			"gender",
			state["submit-steps"]?.biologicalSex,
			"age",
			state["submit-steps"]?.ageGroup
		);
		setTimeout(() => {
			if (p.nextStep) {
