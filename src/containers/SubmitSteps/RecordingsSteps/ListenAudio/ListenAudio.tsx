...
const ListenAudio = ({
	...
	const isShortAudioCollection =
		location?.state?.isShortAudioCollection || false;

	const myState = state?.[storeKey]?.[metadata?.currentLogic];
	const file: File | null = myState
		? myState.recordingFile || myState.uploadedFile
		: null;
	...
	// Memos
	const { fileUrl, fileName } = React.useMemo(() => {
		const out = {
			fileUrl: "",
			fileName: "",
			fileSize: ""
	...
	const handleDoBack = React.useCallback(() => {
		...
		if (location.state && location.state.from) {
			if (isCoughLogic) {
				history.push("/dubai-app/submit-steps/step-record/cough", {
					isShortAudioCollection
				});
			} else if (isBreathLogic) {
				history.push("/dubai-app/submit-steps/step-record/breath");
			} else {
				history.push("/dubai-app/submit-steps/step-record/speech");
			}
	...
	// Memos
	const { currentTime, remainingTime, trackProgress } = React.useMemo(() => {
		const out = {
			currentTime: "0:00",
			remainingTime: "0:00",
			trackProgress: 0
		...
		if (duration) {
			...
			const timeRemaining =
				duration > progressSeconds ? Math.ceil(duration - progressSeconds) : 0;
	...
	return (
		<>
			...
			<MainContainer>
				<Subtitle>{t("recordingsListen:subtitle")}</Subtitle>
				<PlayerContainer>
					<PlayerContainerTop>
						<PlayerTopMiddle>
							<TitleAudioName>{fileName}</TitleAudioName>
						</PlayerTopMiddle>
						<PlayerCrossContainer onClick={handleRemoveFile}>
							<PlayerCross src={CrossSVG} />
						</PlayerCrossContainer>
					</PlayerContainerTop>
					<PlayerContainerBottom>
						<PlayerBottomTop>
							<PlayerBottomTrack playing={playing} progress={trackProgress} />
							<PlayerBottomThumb playing={playing} progress={trackProgress} />
						</PlayerBottomTop>
						<PlayerBottomBottom>
							<PlayerTimeIndicator>{currentTime}</PlayerTimeIndicator>
							<PlayerTimeIndicator>{remainingTime}</PlayerTimeIndicator>
						</PlayerBottomBottom>
					</PlayerContainerBottom>
				</PlayerContainer>
				<PlayerPlayContainer onClick={playing ? handlePause : handlePlay}>
					<PlayerPlayButton>
						<PlayerPlay src={playing ? PauseSVG : PlaySVG} />
					</PlayerPlayButton>
				</PlayerPlayContainer>
			</MainContainer>
			{activeStep && (
